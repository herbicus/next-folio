  ...
  return (
    ...
    >
      <nav className="flex flex-row flex-nowrap items-center justify-start md:justify-between max-w-screen-xl mx-auto">
        <button
          ...
          className={`focus:outline mr-6 md:hidden ${
            isOpen ? 'text-gray-100' : 'text-primary-500'
          }`}
          id="hamburger"
          aria-expanded={isOpen ? 'true' : 'false'}
      ...
      >
        ...
        <ul className="block px-4" role="list">
          {config.nav_links.map((link: any) => (
            <li key={link.label} className="mb-10 last:mb-0" role="listitem">
              <Link href={`${link.href}`}>
                <a
      ...
      <div
        onClick={() => setIsOpen(!isOpen)}
        className={`${
          isOpen ? 'opacity-40 inset-0' : 'opacity-0'
        } fixed bg-black z-10 transition duration-200 ease-in-out`}
      ></div>
    </header>
  );
};
